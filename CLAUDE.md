...
- [x] **1.2: Implement database connection and migrations** (1.5 hours)
  ...
  - TODO (not urgent, deferred 2026-08-22): `db.Init()` opens the connection with SQLite's default `busy_timeout` (0, meaning a second connection hitting a write lock fails immediately with `SQLITE_BUSY` instead of retrying) and no cap on the connection pool (`database/sql` can open multiple physical connections, so two queries within one request aren't guaranteed to land on the same connection). Surfaced while diagnosing a live mobile bug report (favorite/read-later taps not visually updating) - a `PRAGMA busy_timeout` + `conn.SetMaxOpenConns(1)` fix was drafted and verified (tests + a live toggle round-trip) but explicitly not shipped: the actual 500 seen locally that day turned out to be an unrelated dev-environment fluke (the db file got deleted out from under the running server by earlier test scripts), not a real concurrency failure, so this remains an unconfirmed, precautionary fix rather than a proven one. Revisit if a genuine concurrency-related failure (e.g. a toggle tap racing the on-demand fetch's write transaction) is ever actually caught in the wild.
...
- [x] **5.3: On-demand fetch from the dashboard** (added 2026-07-18, built same day)
  ...
  - RESOLVED in part (2026-08-18): the "checking server logs" half of that TODO's motivation - there was previously nowhere durable to even check. `cmd/api/main.go` now has its own `configureLogging` (mirroring `cmd/fetcher`'s), writing slog output to `io.MultiWriter(os.Stdout, logFile)` via a new `LOG_PATH` env var (default `api.log`). Necessary because the on-demand fetch runs `fetcher.FetchAll` inside the `api` server process itself, not the separate `fetcher` binary - without this, its Warn-level per-feed-failure logs only ever reached the container's stdout, which is lost on every redeploy (the container gets recreated). In `docker-compose.yml`, `LOG_PATH` is set to `/data/api.log` - the same already-mounted, already-persistent volume as the SQLite database, not the container's throwaway filesystem. Verified live (ran the server locally with `LOG_PATH` pointed at a temp file, triggered `POST /api/fetch`, confirmed the entry appeared in the file). Side effect discovered along the way: Go's standard `log` package backs onto `slog.SetDefault` since Go 1.21, so this also durably captures the server's other existing `log.Printf` calls (startup messages, `middleware.Logging`'s HTTP request lines) for free, not just `FetchAll`'s output.
  - Still open: the `FailedFeeds`/dashboard-surfacing half of the original TODO - which feed failed and why still isn't shown in the UI, only now durably logged instead of transiently.
...
- [x] **7.1 + 7.2: Above/below-the-fold sections** (merged - the chosen design (issue-v6) uses a 3-tier hierarchy, not the original literal "top 10 in 2 cols, then 4 cols" split)
  ...
  - RESOLVED (2026-08-22, as part of the Saved Posts feature): post title/description text is now cleanly selectable - `PostHero`/`PostMedium`/`PostListItem` no longer wrap the entire card in one `<a>`. Superseded by a deliberate navigation change made to fit favorite/read-later icon buttons onto each card (see Saved Posts feature notes): only the title itself is a link now (`<h2 a>`/`<h3 a>`/`<h4 a>`, `color: inherit` + `text-decoration: none` + hover underline), and the rest of the card is a plain `<article>`/`<div>` - clicking the image or description does nothing, clicking the title opens the article in a new tab. This was a "whole card is clickable" -> "title is clickable" tradeoff, not a text-selection-specific fix, but it resolves this TODO as a side effect.
  - RESOLVED (2026-08-10): `DateNav.svelte` as originally extracted was purely static - no props, no event handlers on either button or the date input, hardcoded placeholder date text - marked `[x]` here despite navigation never actually being wired (caught via a live bug report: "prev, next, and date selector - that does not seem to work"). Fixed by: `web/src/lib/dateUtils.ts`'s `addDaysUTC(dateStr, days)` (pure UTC date arithmetic, tested for month/year rollovers); `DateNav.svelte` now takes a `currentDate` prop and calls SvelteKit's `goto()` on click/change to update a `?date=` query param; `+page.server.ts`'s `load` now destructures `{ url }` (previously took no arguments at all) and calls `getPostsByDate(date)` when present, falling back to the existing `getPostsToday()` otherwise - `getPostsByDate` already existed in `api.ts` but was dead code, never imported anywhere in the frontend. Verified live via a throwaway Playwright script (real browser, real dev server, real seeded data): Previous/Next/date-picker all correctly update the URL, the displayed date, and - confirmed via screenshot - the actual post content shown, not just the label.
  - TODOs (added 2026-08-10, not yet built - `DateNav.svelte` now works functionally, these are polish/follow-up):
    - Add a "Today" button/shortcut to jump straight back to today's issue, rather than only stepping one day at a time via Previous or using the date picker.
    - Prevent navigating "Next" past today - there's no future data to show, so this should be disabled/blocked once `currentDate` reaches today (needs a `todayUTC()`-equivalent check on the client side - see `+page.server.ts`'s own `todayUTC()`, currently server-only, would need to either be duplicated client-side or extracted to `dateUtils.ts` so both can share it).
